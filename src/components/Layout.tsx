...
export function Layout({ children }: { children: React.ReactNode }) {
  // Interactive Mouse Coordinate states for real-world 3D depth parallax
  const [mousePos, setMousePos] = useState({ x: 0, y: 0 });

  useEffect(() => {
    const handleMove = (e: MouseEvent) => {
      // Normalize coordinate metrics to [-1, 1] bounds
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        ...
        <div 
          ...
          style={{
            background: `radial-gradient(circle 480px at ${((mousePos.x + 1) / 2) * 100}% ${((mousePos.y + 1) / 2) * 100}%, rgba(223, 177, 91, 0.08), transparent)`,
          }}
        />
        ...
        <div 
          ...
          style={{
            transform: `translate(${mousePos.x * -35}px, ${mousePos.y * -35}px) rotate(${mousePos.x * 12}deg)`,
        ...
        <div 
          ...
          style={{
            transform: `translate(${mousePos.x * 40}px, ${mousePos.y * 40}px) rotate(${mousePos.y * -15}deg)`,
